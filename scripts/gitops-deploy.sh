    ...
    if [ -f deployment.env ]; then
        ...
        if [ "${TCG_ENABLED:-false}" = "true" ]; then
            if [ -f "/opt/personifi-deployments/infra/jobs/tcg-store.nomad.template" ]; then
                ...
                mkdir -p /opt/tcg-store/data-protection-keys /opt/tcg-store/images/uploaded /opt/tcg-store/files /opt/tcg-store/icons /opt/tcg-store/backups/postgres /opt/tcg-store/backups/media
